...
import {
  ArrowLeft, ArrowRight, Bell, Check, ChevronDown, ChevronRight, CircleHelp, Copy, Edit3,
  Heart, LogOut, MapPin, Package, Plus, Save, Settings, ShieldCheck, ShoppingBag,
  Star, Trash2, Truck, UserRound, X, Activity
} from 'lucide-react';

const orderFilters: Array<'All' | OrderStatus> = ['All', 'Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'];

type AccountSection = 'overview' | 'orders' | 'wishlist' | 'addresses' | 'rewards' | 'recent' | 'notifications' | 'activity' | 'reviews' | 'settings' | 'help';
...
export const AccountPage: React.FC = () => {
  const {
    ...
    discounts, updateProfile, activities
  } = useApp();
  ...
  const submitReview = (productId: number, orderId: string) => {
    saveReview({ productId, orderId, rating: reviewRating, text: reviewText }, reviewEditingId || undefined);
    setReviewProductId(null); setReviewEditingId(null); setReviewText(''); showToast('Review saved.');
  };
  ...
  const navItems: Array<{ id: AccountSection; label: string; icon: React.ReactNode }> = [
    ...
    { id: 'activity', label: 'Activity History', icon: <Activity className="h-4 w-4" /> },
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-[230px_1fr] gap-6 lg:gap-10">
          ...
          <main className="min-w-0">
            ...
            {section === 'overview' && <Overview navigate={navigate} orderCount={customerOrders.length} wishlistCount={wishlist.length} addressCount={addresses.length} recentlyViewed={recentlyViewed} setSelectedProductForQuickView={setSelectedProductForQuickView} addToCart={addToCart} setIsCartOpen={setIsCartOpen} />}
            ...
            {section === 'rewards' && <div className="space-y-5"><SectionTitle title="Discounts & Rewards" subtitle="Offers connected to your client account." /><div className="bg-black text-white dark:bg-white dark:text-black p-6 sm:p-8"><p className="text-[10px] uppercase tracking-[0.25em] opacity-60">Maison Noir rewards</p><p className="font-serif text-4xl mt-3">{customerOrders.filter(order => order.status === 'Delivered').length * 10} points</p><p className="text-xs opacity-70 mt-2">Points are earned from delivered purchases.</p></div><div className="bg-white dark:bg-[#141414] border border-neutral-200 dark:border-neutral-800 p-5"><p className="font-bold text-sm">Available offers</p>{discounts.filter(discount => discount.isActive).length === 0 ? <p className="text-xs text-neutral-500 mt-2">No promotional codes are currently available.</p> : <div className="mt-3 space-y-2">{discounts.filter(discount => discount.isActive).map(discount => <div key={discount.id} className="flex items-center justify-between gap-3 border border-neutral-200 dark:border-neutral-700 p-3"><div><p className="font-bold text-xs">{discount.code}</p><p className="text-[11px] text-neutral-500">{discount.description || `${discount.value}${discount.type === 'percentage' ? '%' : ' off'}`}{discount.expiryDate ? ` · Expires ${discount.expiryDate}` : ''}</p></div><button onClick={() => { navigator.clipboard.writeText(discount.code); showToast('Discount code copied.'); }} className="p-2 border border-neutral-300 dark:border-neutral-700" title="Copy code"><Copy className="h-3.5 w-3.5" /></button></div>)}</div>}</div></div>}
            ...
            {section === 'activity' && <ActivityPanel activities={activities.filter(activity => activity.userId === user.id || activity.userEmail.toLowerCase() === user.email.toLowerCase())} />}

            {section === 'reviews' && <div className="space-y-5"><SectionTitle title="My Reviews" subtitle="Only products from your completed purchases can be reviewed." />{purchasedItems.length === 0 ? <EmptyState title="No eligible reviews" text="After you place an order, purchased pieces will appear here." /> : <div className="space-y-3">{purchasedItems.map(item => { const existing = reviews.find(review => review.productId === item.id); return <div key={`${item.orderId}-${item.id}`} className="bg-white dark:bg-[#141414] border border-neutral-200 dark:border-neutral-800 p-4 flex gap-4"><img src={item.img} alt="" className="h-20 w-16 object-cover bg-neutral-100" /><div className="flex-1"><p className="font-bold text-sm">{item.name}</p><p className="text-xs text-neutral-500 mt-1">Order {item.orderId}</p>{existing ? <div className="mt-3 flex flex-wrap gap-3 text-xs"><button onClick={() => { setReviewProductId(item.id); setReviewEditingId(existing.id); setReviewRating(existing.rating); setReviewText(existing.text); }} className="font-bold underline">Edit review</button><button onClick={() => deleteReview(existing.id)} className="text-rose-600">Delete review</button></div> : <button onClick={() => { setReviewProductId(item.id); setReviewEditingId(null); }} className="mt-3 text-xs font-bold underline">Write a review</button>}</div></div>; })}</div>}{reviewProductId && <div className="fixed inset-0 z-50 modal-safe bg-black/60 flex items-center justify-center p-4"><form onSubmit={e => { e.preventDefault(); const item = purchasedItems.find(product => product.id === reviewProductId); if (item) submitReview(reviewProductId, item.orderId); }} className="bg-white dark:bg-[#141414] p-6 max-w-md w-full"><div className="flex justify-between"><h2 className="font-serif text-2xl">Your review</h2><button type="button" onClick={() => { setReviewProductId(null); setReviewEditingId(null); }}><X className="h-5 w-5" /></button></div><div className="flex gap-2 my-5">{[1, 2, 3, 4, 5].map(r => <button type="button" key={r} onClick={() => setReviewRating(r)} className={r <= reviewRating ? 'text-amber-500' : 'text-neutral-300'}><Star className="h-6 w-6 fill-current" /></button>)}</div><textarea value={reviewText} onChange={e => setReviewText(e.target.value)} required rows={5} placeholder="Share your experience" className="w-full border border-neutral-300 dark:border-neutral-700 bg-transparent p-3 text-sm" /><button className="mt-4 w-full bg-black text-white dark:bg-white dark:text-black py-3 text-xs font-bold uppercase tracking-wider">Save review</button></form></div>}</div>}

            {section === 'settings' && <SettingsPanel user={user} profileName={profileName} setProfileName={setProfileName} updateProfile={updateProfile} openPolicyModal={openPolicyModal} onDelete={() => { setDeletePrompt(false); logout(); setAccountOpen(false); showToast('Account signed out. Contact support to complete account deletion.'); }} />}
...
const Overview: React.FC<any> = ({ navigate, orderCount, wishlistCount, addressCount, recentlyViewed, setSelectedProductForQuickView, addToCart, setIsCartOpen }) => <div className="space-y-6"><div className="grid sm:grid-cols-4 gap-3">{[['Orders', orderCount, 'orders'], ['Saved pieces', wishlistCount, 'wishlist'], ['Addresses', addressCount, 'addresses']].map(([label, count, id]) => <button key={String(id)} onClick={() => navigate(id)} className="text-left bg-white dark:bg-[#141414] border border-neutral-200 dark:border-neutral-800 p-5 hover:border-black dark:hover:border-white"><p className="text-3xl font-serif">{count}</p><p className="text-xs uppercase tracking-wider text-neutral-500 mt-2">{label}</p></button>)}<button onClick={() => setIsCartOpen(true)} className="text-left bg-black text-white dark:bg-white dark:text-black p-5"><ShoppingBag className="h-5 w-5" /><p className="text-xs uppercase tracking-wider mt-4">My Cart</p></button></div><div><SectionTitle title="Recently in view" subtitle="Continue exploring your latest pieces." />{recentlyViewed.length ? <div className="mt-4"><ProductGrid products={recentlyViewed.slice(0, 4)} toggleWishlist={() => undefined} addToCart={addToCart} setSelectedProductForQuickView={setSelectedProductForQuickView} /></div> : <EmptyState title="Your account is ready" text="Explore the collection and save your first piece." />}</div></div>;
...
const NotificationPanel: React.FC = () => { const [preferences, setPreferences] = useState(() => { try { return JSON.parse(localStorage.getItem('mn_notification_preferences') || '[true,true,true,true,true]'); } catch { return [true, true, true, true, true]; } }); const labels = ['Order confirmations','Order status and shipping updates','Delivery notifications','New arrivals and restocks','Discounts and promotions']; const update = (index: number) => setPreferences((current: boolean[]) => { const next = current.map((value, itemIndex) => itemIndex === index ? !value : value); localStorage.setItem('mn_notification_preferences', JSON.stringify(next)); return next; }); return <div className="space-y-5"><SectionTitle title="Notifications" subtitle="Choose what you want to hear from the atelier." /><div className="bg-white dark:bg-[#141414] border border-neutral-200 dark:border-neutral-800 divide-y divide-neutral-200 dark:divide-neutral-800">{labels.map((label, index) => <label key={label} className="flex items-center justify-between gap-4 p-4 text-sm"><span>{label}</span><input type="checkbox" checked={preferences[index]} onChange={() => update(index)} /></label>)}</div></div>; };
const ActivityPanel: React.FC<{ activities: import('../types').CustomerActivity[] }> = ({ activities }) => <div className="space-y-5"><SectionTitle title="Activity History" subtitle="A record of actions taken on your account." />{activities.length === 0 ? <EmptyState title="No activity yet" text="Your account actions will appear here." /> : <div className="bg-white dark:bg-[#141414] border border-neutral-200 dark:border-neutral-800 divide-y divide-neutral-200 dark:divide-neutral-800">{activities.map(activity => <div key={activity.id} className="p-4 flex items-start gap-3"><Activity className="h-4 w-4 mt-0.5 text-indigo-600 shrink-0" /><div><p className="text-sm">{activity.description}</p><p className="text-[10px] text-neutral-400 mt-1">{new Date(activity.createdAt).toLocaleString()}</p></div></div>)}</div>}</div>;

const SettingsPanel: React.FC<any> = ({ user, profileName, setProfileName, updateProfile, openPolicyModal, onDelete }) => <div className="space-y-5"><SectionTitle title="Account Settings" subtitle="Your connected account and privacy controls." /><div className="bg-white dark:bg-[#141414] border border-neutral-200 dark:border-neutral-800 p-5 space-y-4"><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-emerald-600" /><div><p className="font-bold text-sm">Connected Google account</p><p className="text-xs text-neutral-500 mt-1">{user.email}</p></div></div><div className="border-t border-neutral-200 dark:border-neutral-800 pt-4"><label className="text-xs text-neutral-500">Display name</label><div className="flex gap-2 mt-2"><input value={profileName} onChange={event => setProfileName(event.target.value)} className="flex-1 border border-neutral-300 dark:border-neutral-700 bg-transparent px-3 py-2 text-xs" /><button onClick={() => updateProfile(profileName)} className="px-3 py-2 bg-black text-white dark:bg-white dark:text-black text-xs font-bold"><Save className="inline h-3.5 w-3.5 mr-1" /> Save</button></div></div><div className="border-t border-neutral-200 dark:border-neutral-800 pt-4 grid sm:grid-cols-2 gap-3"><button onClick={() => openPolicyModal('legal')} className="text-left text-xs text-neutral-500 hover:text-black dark:hover:text-white">Privacy Policy <ChevronRight className="inline h-3 w-3" /></button><button onClick={() => openPolicyModal('legal')} className="text-left text-xs text-neutral-500 hover:text-black dark:hover:text-white">Terms & Conditions <ChevronRight className="inline h-3 w-3" /></button><button className="text-left text-xs text-neutral-500">Help & Support <ChevronRight className="inline h-3 w-3" /></button><button onClick={onDelete} className="text-left text-xs text-rose-600">Delete account permanently <ChevronRight className="inline h-3 w-3" /></button></div></div></div>;
